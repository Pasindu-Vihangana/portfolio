...
type BannerState = "available" | "booked" | "vacation" | "collab" | "hidden";

interface BannerStateConfig {
  dotColor: string;
  text: string;
  buttonText: string;
  buttonLink: string;
...
const DEFAULT_STATE: BannerState = "available";

const BANNER_CONFIGS: Record<Exclude<BannerState, "hidden">, BannerStateConfig> = {
  available: {
    dotColor: "bg-emerald-500",
    text: "Available for remote contracts",
    buttonText: "Discuss Options",
    buttonLink: "/contact",
    showWhatsApp: true,
    whatsAppText: "Hi Pasindu, I saw your portfolio and would like to discuss remote contract opportunities!",
  },
  booked: {
    dotColor: "bg-amber-500",
    text: "Fully booked (Next availability: Q3)",
    buttonText: "Get Waitlisted",
    buttonLink: "/contact",
    showWhatsApp: true,
    whatsAppText: "Hi Pasindu, I saw you are fully booked but wanted to get in touch for future projects!",
  },
  vacation: {
    dotColor: "bg-blue-500",
    text: "Currently away on vacation",
  ...
  collab: {
    ...
    text: "Looking for robotics collaborators",
    buttonText: "Let's Collab",
    buttonLink: "/contact",
    showWhatsApp: true,
    whatsAppText: "Hi Pasindu, I'm interested in collaborating on a robotics or embedded systems project with you!",
  },
};
...
export default function FloatingBanner() {
  ...
  const stateEnv = process.env.NEXT_PUBLIC_BANNER_STATE;
  const bannerState: BannerState = (
    ...
    stateEnv === "vacation" || 
    stateEnv === "collab"
  ) ? stateEnv : DEFAULT_STATE;

  const isHidden = bannerState === "hidden";
  const baseConfig = BANNER_CONFIGS[bannerState === "hidden" ? "available" : bannerState];

  // Resolve config overrides from environment variables
  const dotColor = process.env.NEXT_PUBLIC_BANNER_DOT_COLOR || baseConfig.dotColor;
  const text = process.env.NEXT_PUBLIC_BANNER_TEXT || baseConfig.text;
  const buttonText = process.env.NEXT_PUBLIC_BANNER_BTN_TEXT || baseConfig.buttonText;
  const buttonLink = process.env.NEXT_PUBLIC_BANNER_BTN_LINK || baseConfig.buttonLink;
  
  const rawShowWhatsApp = process.env.NEXT_PUBLIC_BANNER_SHOW_WHATSAPP;
  const showWhatsApp = rawShowWhatsApp !== undefined
    ? rawShowWhatsApp === "true"
    : baseConfig.showWhatsApp;
    
  const whatsAppText = process.env.NEXT_PUBLIC_BANNER_WHATSAPP_TEXT || baseConfig.whatsAppText;
  const whatsappUrl = `https://wa.me/94763498429?text=${encodeURIComponent(whatsAppText)}`;
